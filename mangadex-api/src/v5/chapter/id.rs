...
impl IdEndpoint{
    ...
    pub fn get(&self) -> GetChapterBuilder{
        GetChapterBuilder::default().http_client(self.http_client.clone()).chapter_id(self.id)
    }
    pub fn put(&self) -> UpdateChapterBuilder{
        UpdateChapterBuilder::default().http_client(self.http_client.clone()).chapter_id(self.id)
    }
    pub fn delete(&self) -> DeleteChapterBuilder{
        DeleteChapterBuilder::default().http_client(self.http_client.clone()).chapter_id(self.id)
    }
}