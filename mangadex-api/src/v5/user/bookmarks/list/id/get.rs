//! Builder for checking if the logged-in user bookmarked a custom list.
//!
...
///
/// Makes a request to `GET /user/bookmarks/list/{id}`.
#[cfg_attr(
...
endpoint! {
    GET ("/user/bookmarks/list/{}", list_id),
    #[no_data auth] HaveBookMarkedCustomList,
    #[no_send] Result<IsFollowingResponse>
}

builder_send! {
    #[builder] HaveBookMarkedCustomListBuilder,
    IsFollowingResponse
}
...
mod tests {
    ...
    async fn user_follows_custom_list() -> anyhow::Result<()> {
        ...
        Mock::given(method("GET"))
            .and(path_regex(r"/user/bookmarks/list/[0-9a-fA-F-]+"))
